    ...
    public class AddPostActivity : BaseActivity , SlidingPaneLayout.IPanelSlideListener, SlidingUpPanelLayout.IPanelSlideListener, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, MaterialDialog.IInputCallback
    {
        ...
        public static void DownloadString()
        {
            try
            {
                ...
                string url = "https://playtubescript.com/";
                ...
                var keywwordNodes2 = htmlDoc.DocumentNode.SelectNodes("//meta[translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') = 'og:description']");
                if (keywwordNodes2 != null)
                {
                    foreach (var keywordNode in keywwordNodes2)
                    {
                        string content = keywordNode.GetAttributeValue("content", "");
                        if (!string.IsNullOrEmpty(content))
                        {
                ...
                var image2 = htmlDoc.DocumentNode.SelectNodes("//meta[translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz') = 'og:image']");
                if (image2 != null)
                {
                    //foreach (var keywordNode in image2)
                    //{
                    //    string content = keywordNode.GetAttributeValue("content", "");
                    //    if (!string.IsNullOrEmpty(content))
                    //    {
                ...
                if (anchorNodes != null)
                {
                    ...
                    {
                        Console.WriteLine($"{anchorNode.InnerText} - {anchorNode.GetAttributeValue("href", "")}");
                    }
                }
            }
            catch (Exception e)
