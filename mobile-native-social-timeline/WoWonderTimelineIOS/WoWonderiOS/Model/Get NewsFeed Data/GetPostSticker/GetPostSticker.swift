...
class GetPostSticker :AddReactionDelegate,SharePostDelegate{
    ...
    func getPostSticker (targetController :UIViewController,tableView : UITableView, indexpath:IndexPath, postFile : String, array : [[String:Any]],stackViewHeight: CGFloat,viewHeight: CGFloat, isHidden: Bool, viewColor :UIColor) -> UITableViewCell {
        ...
        if let commentsCount = index["post_comments"] as? String{
            cell.commentCountBtn.setTitle("\(NSLocalizedString("Comments", comment: "Comments"))\(" ")\(commentsCount)", for: .normal)
        }
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 6)) as! GifImageCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 6)) as! GifImageCell
