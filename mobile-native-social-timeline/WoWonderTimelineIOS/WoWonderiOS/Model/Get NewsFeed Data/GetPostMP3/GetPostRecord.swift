...
class GetPostRecord :AddReactionDelegate,SharePostDelegate {
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 6)) as! MusicCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 6)) as! MusicCell
...
//        }
    ...
    @IBAction func PlayBtn (sender:UIButton){
        let cell = self.tableView.cellForRow(at: IndexPath(row: sender.tag, section: 6)) as! MusicCell
    ...
    @objc func checkDownload(){
        if self.isDownlodingComplete == true{
            if self.isPlay == true{
                let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectIndex ?? 0, section: 6)) as! MusicCell
                cell.indicatorView.stopAnimating()
                self.isPlay = false
            }
            self.timer.invalidate()
