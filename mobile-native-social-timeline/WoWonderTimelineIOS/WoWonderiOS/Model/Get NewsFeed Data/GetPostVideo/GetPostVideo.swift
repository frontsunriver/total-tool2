...
class GetPostVideo :AddReactionDelegate,SharePostDelegate {
    ...
    @IBAction func NormalTapped(gesture: UIGestureRecognizer){
        ...
        case .online(.wwan), .online(.wiFi):
            let cell = self.tableView.cellForRow(at: IndexPath(row: gesture.view!.tag, section: 6)) as! VideoCell
    ...
    func addReaction(reation: String) {
        let cell = self.tableView.cellForRow(at: IndexPath(row: self.selectedIndex, section: 6)) as! VideoCell
...
//        }
    ...
    @objc func playVideo (_ sender : UIButton){
        ...
        let cell = tableView.cellForRow(at: IndexPath(row: indexPath.row, section: 6)) as! VideoCell
        cell.videoView.play()
