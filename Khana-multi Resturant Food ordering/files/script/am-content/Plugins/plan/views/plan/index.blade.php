		...
		<div class="row mb-30">
			...
			<div class="col-lg-6">
				<div class="add-new-btn">
					<a href="{{ route('admin.plan.create') }}" class="btn f-right btn btn-primary">{{ __('Add New') }}</a>
				</div>
			</div>
		</div>
		<div class="cart-filter mb-20">
			<a href="{{ route('admin.plan.index') }}">{{ __('All') }} <span>({{ App\Plan::where('status','!=',0)->count() }})</span></a>
			<a href="?st=1">{{ __('Published') }} <span>({{ App\Plan::where('status',1)->count() }})</span></a>
			<a href="?st=2">{{ __('Drafts') }} <span>({{ App\Plan::where('status',2)->count() }})</span></a>
			<a href="?st=trash" class="trash">{{ __('Trash') }} <span>({{ App\Plan::where('status',0)->count() }})</span></a>
		</div>
		<div class="card-action-filter">
			<form method="post" id="basicform" action="{{ route('admin.plans.destroy') }}">
				@csrf
				<div class="row">
					<div class="col-lg-6">
						<div class="d-flex">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								{{ $post->name }}
								<div class="hover">
									<a href="{{ route('admin.plan.edit',$post->id) }}">{{ __('Edit') }}</a>

									<a href="{{ route('admin.plan.show',$post->id) }}" class="last">{{ __('View') }}</a>
								</div>
							</td>
							<td>
								{{ $post->s_price }}
