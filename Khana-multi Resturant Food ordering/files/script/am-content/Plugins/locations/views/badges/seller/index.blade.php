...
<div class="card"  >
	<div class="card-body">
		...
		<div class="cart-filter mb-20">
			<a href="{{ route('admin.badge.index','type=seller') }}">{{ __('All') }} <span>({{ App\Terms::where('type',3)->where('status','!=',0)->count() }})</span></a>
			<a href="?st=1&type=seller">{{ __('Published') }} <span>({{ App\Terms::where('type',3)->where('status',1)->count() }})</span></a>
			<a href="?st=2&type=seller">{{ __('Drafts') }} <span>({{ App\Terms::where('type',3)->where('status',2)->count() }})</span></a>
			<a href="?st=trash&type=seller" class="trash">{{ __('Trash') }} <span>({{ App\Terms::where('type',3)->where('status',0)->count() }})</span></a>
		</div>
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th class="am-title">{{ __('Amount Of Earning') }}</th>
